...
'''
        ...
        self.assertEqual([i for i in parser], [{'a': 'one', 'c': '3', 'b': 'two'}])
    ...
    def test_b(self):
        parser = CSVReader(data_source=FileReader('test_data/simple.csv'), cleaner=FuncMapper(StringCleaner.safe_int))
        self.assertEqual([i for i in parser], [{'a': None, 'c': 3, 'b': None}])

    def test_c(self):
        parser = CSVReader(data_source=FileReader('test_data/simple.csv'), cleaner=FuncDictMapper({'c': StringCleaner.safe_int}))
        self.assertEqual([i for i in parser], [{'a': 'one', 'c': 3, 'b': 'two'}])

    def test_d(self):
        parser = CSVReader(data_source=FileReader('test_data/simple.csv'), cleaner=FuncDictMapper({'c': StringCleaner.safe_int}))
        self.assertEqual(parser(), [{'a': 'one', 'c': 3, 'b': 'two'}])
    ...
    def test_g(self):
        ...
        self.assertEqual([i['Symbol'] for i in DictTransform(data_source=CSVReader(data_source=FileReader('test_data/numbers.csv')), remap_fields={'SMBL': 'Symbol'})], ['AAPL', 'GOOG', 'NFLX'])
