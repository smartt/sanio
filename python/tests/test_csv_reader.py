...
#         self.assertEqual([i for i in parser], [{'a': 'one', 'b': 'two', 'c': '3'}])
    ...
    def test_a(self):
        parser = CSVReader(data_source=FileReader(filename='tests/test_data/simple.csv'))
        self.assertEqual([i for i in parser], [{'a': 'one', 'b': 'two', 'c': '3'}])

    def test_b(self):
        parser = CSVReader(data_source=FileReader(filename='tests/test_data/simple.csv'), cleaner=FuncMapper(StringCleaner.safe_int))
        self.assertEqual([i for i in parser], [{'a': None, 'b': None, 'c': 3}])

    def test_c(self):
        parser = CSVReader(data_source=FileReader(filename='tests/test_data/simple.csv'), cleaner=FuncMapper({'c': StringCleaner.safe_int}))
        self.assertEqual([i for i in parser], [{'a': 'one', 'b': 'two', 'c': 3}])
    ...
    def test_e(self):
        parser = CSVReader(data_source=FileReader(filename='tests/test_data/numbers.csv'))
    ...
    def test_g(self):
        parser = DictTransform(data_source=CSVReader(data_source=FileReader(filename='tests/test_data/numbers.csv')), remap_fields={'SMBL': 'Symbol'})
        self.assertEqual([i['Symbol'] for i in parser], ['AAPL', 'GOOG', 'NFLX'])

        self.assertEqual([i['Symbol'] for i in DictTransform(data_source=CSVReader(data_source=FileReader(filename='tests/test_data/numbers.csv')), remap_fields={'SMBL': 'Symbol'})], ['AAPL', 'GOOG', 'NFLX'])
